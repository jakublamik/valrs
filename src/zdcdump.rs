...
fn build_parameter_map(zdcs: &[&Zdc]) -> HashMap<(String, String), HashMap<String, Vec<String>>> {
...
fn compare_parameters_with_target_in_all_zdcs(
    ...
    for (diag_addr, zdcs) in zdc_groups {
        ...
        let parameter_map = build_parameter_map(&zdcs);
        ...
        if !parameter_map.is_empty() {
            ...
            process_parameter_map(&diag_addr, parameter_map, get_target_value_by_name);
        }
    }
}
...
fn compare_parameters(zdc: &Zdc, zdc_dump_config: &ZdcDumpConfig) {
